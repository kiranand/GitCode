...
namespace FormBasedTCPListenOutstation
{
    ...
    class TCPListensadfsadfsdafsdf
    {
        ...
        IPAddress tempIPAddress = IPAddress.Parse("192.168.1.125");
        ...
        public void setLocalAddr()
        {
            ...
            for(int i=0;i<count;i++)
            {
                ...
                //stationConsole.Text += "addr Byte" + addrBytes[i] + Environment.NewLine;
                if(addrBytes[0]==0xC0)
                {
                    localAddr = addr[i];
                    break;
            ...
            stationConsole.Text += "SharpPcap {0}, Example1.IfList.cs" + Environment.NewLine;
/*
            ...
            { 
                if (var == FormBasedTCPListenOutstation.variationID.V1)
                {
                    ...
                    else if (prefixRange == FormBasedTCPListenOutstation.prefixAndRange.NoIndexOneOctetStartStop)
                    {
                        ...
                        //params should be in the following order
                        //Confirm, Unsolicited, function, group, variation, prefixQualifier, [range] OR [start index, stop index]
                        responseAPDU.buildAPDU(ref dnpResponse, 0x00, 0x00, 0x81, (byte)group, (byte)var, (byte)prefixRange, startIndex, stopIndex,0,1,1);
